...
Task 004D provides a separate shadow CLI for bounded TwitterAPI.io and SocialData quality and cost comparison against stored Official X data. Task 004D.2.2 gives each provider its own discovery and cost contract, binds approval to exact benchmark tasks or selected IDs, durably saves paid HTTP 200 responses before normalization, applies TwitterAPI.io pacing without retry, uses exact inclusive SocialData `max_id` continuation, and distinguishes a documented maximum from SocialData's unknown page-size worst case. It also provides planning-only selection and fixture comparison for a future 50-ID lookup. The shadow path never changes the production collector or `sync_state`. A separate confirmation-gated offline command recovered the 11 already-paid Official X pages retained before HTTP 402 through the production Post mapping without external calls. LLM integration, Telegram, and publication are not implemented. All publication remains a mandatory human action.

Every future usage-based external call requires a zero-cost preflight and explicit approval of the exact plan. The preflight identifies the input identity, provider, endpoint, purpose, request cap, expected billable resources and cost, conservative estimate, documented response maximum when available, and enforcing guard. An unknown response maximum must remain explicit and cannot be described as a technical hard dollar cap. Existing local data is reused before equivalent data is purchased.
...
- Last completed precision correction - Task 005D.1 - Vocabulary Precision Review
...
Both commands are zero-cost: they read the stored benchmark, do not load provider keys, make no provider request, and return `execution_authorized=false`. Discovery approval includes the exact benchmark identity, canonical SearchTasks, strategy parameters, and amount; direct-ID approval includes the exact selected IDs. TwitterAPI.io uses a documented 20-result reserve and local pacing. SocialData places `since_id` and inclusive `max_id` inside the Search query, uses a separately approved request cap, and labels its 20-result page assumption as conservative because no maximum is assumed. The runner accepts at most $0.25 per provider, while defaults remain $0.10 for discovery and $0.02 for planning-only direct-ID. The ceiling is not approval. Direct-ID API execution is not implemented. Official X remains production. See [the Task 004D operating and report document](docs/x-provider-shadow-spike.md).
