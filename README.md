...
Both commands are zero-cost: they read the stored benchmark, do not load provider keys, make no provider request, and return `execution_authorized=false`. Discovery approval includes the exact benchmark identity, canonical SearchTasks, and strategy parameters; direct-ID approval includes the exact selected IDs. TwitterAPI.io uses a documented 20-result reserve and local pacing. SocialData places `since_id` and `max_id` inside the Search query, uses a separately approved request cap, and labels its 20-result page assumption as conservative because no maximum is assumed. Direct-ID API execution is not implemented. Official X remains production. See [the Task 004D operating and report document](docs/x-provider-shadow-spike.md).
