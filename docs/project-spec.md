...
The current implementation includes the repository foundation, documentation, prompts, a local CLI, durable PostgreSQL storage, the isolated read-only X API access-spike probe, and the live-validated bounded Task 004A and Task 004B collector work. Task 004B extends this collector to prefer `note_tweet.text` for long Posts, retain returned referenced Post and media metadata in the original Post's `raw_json`, and expose manual PostgreSQL review views. The collector is manually started, uses a refresh token stored only in the ignored local `.env`, fetches home or `@Ethplorer` mentions through X API v2, excludes simple home reposts, and upserts returned Posts, run records, usage estimates, and independent source checkpoints into PostgreSQL. Its safe default is one page and 20 Posts. An explicit bounded `--refresh-existing` mode omits `since_id`, anchors the request at the stored checkpoint with `until_id`, refreshes stored rows by Post ID, and leaves the operational checkpoint unchanged. It does not perform a historical backfill. An initial limited run establishes a documented current baseline without historical backfill; an incremental run does not advance its checkpoint when pagination remains incomplete, response-level partial errors exist, or duplicate IDs make completeness uncertain. Live validation confirmed real home persistence, original, reply, and quote mapping, full long-Post text, expanded referenced context, media metadata, post-ID deduplication, checkpoint preservation during refresh, refresh-token reuse and rotation, and a successful empty mentions run. Full Stage 3 pagination, missed-window recovery, and compliance automation remain unimplemented. Model calls, Signals, Opportunities, Telegram, scheduling, X write operations, automatic unfollow, media downloads, and publication are not implemented.
