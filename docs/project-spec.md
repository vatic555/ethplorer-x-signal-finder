...
The current implementation includes the repository foundation, documentation, prompts, a local CLI, durable PostgreSQL storage, the isolated read-only X API access-spike probe, and the live-validated bounded Task 004A collector. The collector is manually started, uses a refresh token stored only in the ignored local `.env`, fetches home or `@Ethplorer` mentions through X API v2, excludes simple home reposts, and upserts returned Posts, run records, usage estimates, and independent source checkpoints into PostgreSQL. Its safe default is one page and 20 Posts. An initial limited run establishes a documented current baseline without historical backfill; an incremental run does not advance its checkpoint when pagination remains incomplete, response-level partial errors exist, or duplicate IDs make completeness uncertain. Live Task 004A validation confirmed real home persistence, original, reply, and quote mapping, post-ID deduplication, checkpoint reuse, refresh-token reuse and rotation, and a successful empty mentions run. Full Stage 3 pagination, missed-window recovery, and compliance automation remain unimplemented. Model calls, Signals, Opportunities, Telegram, scheduling, and publication are not implemented.
