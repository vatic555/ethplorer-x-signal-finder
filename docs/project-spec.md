...
`knowledge/sources/posts/` is the canonical location for the 17 current Ethplorer Markdown articles and their managed local article media. They must not be moved or renamed merely to match a proposed product-oriented layout. Each uses `source_type = ethplorer_article`, which is distinct from the future first-party X editorial corpus.
