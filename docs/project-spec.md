...
Task 004D shadow usage is diagnostic and local. Its completed comparison reused an already-collected Official X window and accepted neither third-party provider. Any continuation or repeat must first use the existing 214 incoming Official X Posts and other suitable local artifacts as the benchmark instead of purchasing another full window. TwitterAPI.io and SocialData must begin with a separately approved sample of approximately 20 to 50 Posts or the smallest sufficient window. Each call requires the general preflight and technical hard ceiling above, and a larger comparison requires new approval. Raw responses and normalized content remain under ignored `data/runtime/` storage; committed reports contain aggregate metrics only. The shadow command never writes PostgreSQL, `posts`, `sync_state`, or production usage events. Official X remains the production source.
