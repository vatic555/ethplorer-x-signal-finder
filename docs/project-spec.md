...
First-party X sync records separate inventory, Ethplorer, and Binplorer usage operations. Within each source sync it separately tracks distinct primary Post resources, expanded Post resources, reference-completion Post resources, their deduplicated total, User resources returned through expansions or lookups, Media resources, and request count. Inventory User resources remain a separate operation. Configurable standard estimates are $0.005 per Post Read, $0.010 per User Read, and $0.005 per Media Read. The first-party cost guard uses the conservative sum of all three estimated resource classes and can overshoot by one completed response. `reported_cost` remains NULL until Developer Console reconciliation. Historical Task 005C estimates produced before this accounting correction are not retroactively reconstructed.
...
Task 004D shadow usage is diagnostic and local. It may read one approximately 24-hour Official X home-timeline benchmark - including an already-collected read-only window when a fresh Official X request is unavailable - and query only the benchmark's active authors through TwitterAPI.io and SocialData. Each third-party provider has a hard $0.10 spend ceiling. Raw responses and normalized content remain under ignored `data/runtime/` storage; the committed report contains aggregate metrics only. The shadow command never writes PostgreSQL, `posts`, `sync_state`, or production usage events. Credit exhaustion is recorded as `incomplete_due_to_credit`, not as an automatic quality failure. The completed spike accepted neither third-party provider and leaves Official X as the production source.
