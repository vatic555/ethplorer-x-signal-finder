...
Task 005D reviews all 17 static sources, records 11 evidence-backed capabilities, assigns 12 canonical Ethplorer article URLs deterministic source identity, and adds a manually reviewed 76-trigger derivative vocabulary informed by static evidence, non-repost first-party authored language, separately counted referenced context, and exact article links. It commits no raw X content or per-Post mapping and does not implement runtime matching. Task 005E dynamic analytics and Task 006 relevance processing remain unimplemented.
...
Every usage-based or potentially paid external call requires a zero-cost preflight and explicit user approval before execution. The preflight must state the provider and endpoint, purpose, expected requests and billable resources, known unit price, expected cost, conservative maximum cost, and the technical hard guard that prevents spending above that maximum. Approval applies only to that ceiling. More pages, time, Posts, tokens, retries, provider calls, or spending require a new preflight and approval. Unknown or unreliable pricing blocks the call unless the owner separately approves an experiment with a stated and technically enforced hard dollar cap.
...
Task 004D shadow usage is diagnostic and local. Its completed comparison reused an already-collected Official X window and accepted neither third-party provider. A preceding fresh Official X attempt retained 11 successful paid pages before a twelfth request returned HTTP 402; the owner-reported charge was 1,133 Post Reads and $5.665. Any continuation or repeat must use existing incoming Official X Posts and suitable local artifacts as the benchmark instead of purchasing another full window. TwitterAPI.io and SocialData must begin with a separately approved sample of approximately 20 to 50 Posts or the smallest sufficient window. Each call requires the general preflight and technical hard ceiling above, and a larger comparison requires new approval. Raw responses and normalized content remain under ignored `data/runtime/` storage; committed reports contain aggregate metrics only. The normal shadow run never writes PostgreSQL, `posts`, `sync_state`, or production usage events. Task 004D.2 disables fresh Official X retrieval in provider discovery, adds a zero-cost plan bound to a SHA-256 approval digest, refuses execution when its initial author coverage cannot fit the unchanged hard cap, and reserves one full provider page before every request without automatic retries or limit increases. TwitterAPI.io recursively bisects overflowing time windows down to a minimum slice and blocks repeated windows. SocialData separately traverses advancing cursors and falls back to decreasing `max_id` while blocking repeated cursor, ID, or page state. Both deduplicate by canonical `post_id` and return explicit incomplete status whenever coverage cannot be proven. Planning-only direct-ID selection and offline comparison cover availability, exact and long text, replies, quotes, referenced context, and media for up to 50 locally known IDs; provider direct-ID API execution is not implemented. The one separately confirmation-gated offline recovery command may upsert already-paid pages to `posts` using canonical `post_id` deduplication and production mapping, but it makes no external request and never changes `sync_state`. Official X remains the production source.
